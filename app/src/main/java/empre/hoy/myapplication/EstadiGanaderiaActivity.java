...
//        xAxis.setTypeface(Typeface.createFromAsset(getAssets(), "font/sofia_pro_medium.otf"));
    ...
    public static void obtenerVentas(ArrayList<VentasReporte> ventas) {
        if (ventas.size() > 0) {
            ...
            for (int i = 0; i < ventas.size(); i++) {
                data.add(new Data(0, (float) ventas.get(i).getCantidad(), "Revisión " + (i + 1)));
            }
            xAxis.setValueFormatter(new EstadiGanaderiaActivity.ValueFormatter() {
                @Override
    ...
    private static void setData(List<Data> dataList) {
