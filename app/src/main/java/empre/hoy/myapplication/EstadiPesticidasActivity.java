...
public class EstadiPesticidasActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ivFiltro.setOnClickListener(v -> {
            startActivity(new Intent(EstadiPesticidasActivity.this, EstadisticasGeneralActivity.class));
        });
    }
...
//        xAxis.setTypeface(Typeface.createFromAsset(getAssets(), "font/sofia_pro_medium.otf"));
        ...
        webService.consulta(params, "obtener_cantidad_ventas_pesticidas.php");
    }

    public static void obtenerVentas(ArrayList<VentasReporte> ventas) {
        if (ventas.size() > 0) {
            final List<EstadiPesticidasActivity.Data> data = new ArrayList<>();
            for (int i = 0; i < ventas.size(); i++) {
                data.add(new EstadiPesticidasActivity.Data(0, (float) ventas.get(i).getCantidad(), "Revisión " + (i + 1)));
            }
            xAxis.setValueFormatter(new EstadiPesticidasActivity.ValueFormatter() {
                @Override
    ...
    private static void setData(List<EstadiPesticidasActivity.Data> dataList) {
        ArrayList<BarEntry> values = new ArrayList<>();
        List<Integer> colors = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            EstadiPesticidasActivity.Data d = dataList.get(i);
        ...
        if (bchVentas.getData() != null && bchVentas.getData().getDataSetCount() > 0) {
            ...
            data.setValueFormatter(new EstadiPesticidasActivity.ValueFormatter());
