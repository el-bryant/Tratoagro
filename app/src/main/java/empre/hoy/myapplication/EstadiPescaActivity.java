...
public class EstadiPescaActivity extends AppCompatActivity {
    static XAxis xAxis;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_estadipesca);
        ...
        ivFiltro.setOnClickListener(v -> {
            startActivity(new Intent(EstadiPescaActivity.this, EstadisticasGeneralActivity.class));
        });
    }
...
//        xAxis.setTypeface(Typeface.createFromAsset(getAssets(), "font/sofia_pro_medium.otf"));
        ...
        webService.consulta(params, "obtener_cantidad_ventas_pesca.php");
    }

    public static void obtenerVentas(ArrayList<VentasReporte> ventas) {
        if (ventas.size() > 0) {
            final List<EstadiPescaActivity.Data> data = new ArrayList<>();
            for (int i = 0; i < ventas.size(); i++) {
                data.add(new EstadiPescaActivity.Data(0, (float) ventas.get(i).getCantidad(), "Revisión " + (i + 1)));
            }
            xAxis.setValueFormatter(new EstadiPescaActivity.ValueFormatter() {
                @Override
    ...
    private static void setData(List<EstadiPescaActivity.Data> dataList) {
        ArrayList<BarEntry> values = new ArrayList<>();
        List<Integer> colors = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            EstadiPescaActivity.Data d = dataList.get(i);
        ...
        if (bchVentas.getData() != null && bchVentas.getData().getDataSetCount() > 0) {
            ...
            data.setValueFormatter(new EstadiPescaActivity.ValueFormatter());
