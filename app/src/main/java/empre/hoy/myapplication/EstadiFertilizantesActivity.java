...
public class EstadiFertilizantesActivity extends AppCompatActivity {
    static XAxis xAxis;
...
//        xAxis.setTypeface(Typeface.createFromAsset(getAssets(), "font/sofia_pro_medium.otf"));
        ...
        webService.consulta(params, "obtener_cantidad_ventas_fertilizantes.php");
    }

    public static void obtenerVentas(ArrayList<VentasReporte> ventas) {
        if (ventas.size() > 0) {
            final List<EstadiFertilizantesActivity.Data> data = new ArrayList<>();
            for (int i = 0; i < ventas.size(); i++) {
                data.add(new EstadiFertilizantesActivity.Data(0, (float) ventas.get(i).getCantidad(), "Revisión " + (i + 1)));
            }
            xAxis.setValueFormatter(new EstadiFertilizantesActivity.ValueFormatter() {
                @Override
    ...
    private static void setData(List<EstadiFertilizantesActivity.Data> dataList) {
        ArrayList<BarEntry> values = new ArrayList<>();
        List<Integer> colors = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            EstadiFertilizantesActivity.Data d = dataList.get(i);
        ...
        if (bchVentas.getData() != null && bchVentas.getData().getDataSetCount() > 0) {
            ...
            data.setValueFormatter(new EstadiFertilizantesActivity.ValueFormatter());
